...
const SongBox = ({ song, className = '', children }: SongBoxProps) => {
  ...
  return (
    <article
      className={`group relative flex min-h-[120px] min-w-[120px] cursor-pointer flex-col justify-between overflow-hidden rounded-2xl sm:rounded-3xl border border-zinc-200/80 bg-white p-4 sm:p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg active:scale-[0.98] sm:hover:-translate-y-1.5 sm:hover:shadow-xl sm:active:scale-100 hover:shadow-zinc-200/50 dark:border-zinc-800 dark:bg-zinc-900/80 dark:hover:border-zinc-700 dark:hover:shadow-zinc-950/50 ${className}`.trim()}
    >
      <div>
        <div className="mb-3 flex items-start justify-between gap-2 sm:mb-4 sm:gap-3">
          ...
          <div className="shrink-0 rounded-full bg-zinc-100 px-2.5 py-1 text-[10px] font-bold tabular-nums tracking-widest text-zinc-600 sm:px-3 sm:py-1.5 sm:text-[11px] dark:bg-zinc-800 dark:text-zinc-300">
            {duration ?? '—'}
          </div>
        </div>
        <div className="flex items-center gap-1.5 text-xs text-zinc-600 sm:gap-2 sm:text-sm dark:text-zinc-400">
          <Disc3
            className="h-3.5 w-3.5 shrink-0 text-zinc-400 sm:h-4 sm:w-4 dark:text-zinc-500 group-hover:animate-[spin_3s_linear_infinite]"
            aria-hidden="true"
          />
